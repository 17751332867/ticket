...
def xiecheng(dcity, acity,yyyy,mm,dd):

    # 打开数据库连接
    db = pymysql.connect(host="localhost", user="root", password="123456", database="ticket",charset="utf8")
    ...
    table = PrettyTable(["Airline", "FlightNumber", "DepartureDate", 'ArrivalDate', 'PunctualityRate', 'standardPrices','cabinClass','businessClass','firstClass'])
    ...
    for route in routeList:
        if len(route.get('legs')) == 1:
            ...
            # info['DepartureDate']=info['DepartureDate'].replace(":","-")
            info['ArrivalDate'] = flight.get('arrivalDate')[-8:-3]
            # info['ArrivalDate']=info['ArrivalDate'].replace(":","-")
            info['PunctualityRate'] = flight.get('punctualityRate')
            try:
                info['standardPrices'] = legs.get('characteristic').get('standardPrices')
            except:
                continue
            info['economyClass']=0
            info['businessClass'] = 0
            info['firstClass'] = 0
            if info['standardPrices']:
                for price in info['standardPrices']:
                    if price['cabinClass'] == 'Y':
                        info['economyClass'] = price['price']
                    if price['cabinClass'] == 'C':
                        info['businessClass'] = price['price']
                    if price['cabinClass'] == 'F':
                        info['firstClass'] = price['price']
            table.add_row(info.values())
            sql="insert into crawler(airline,flightNumber,departureDate,arrivalDate,economyClass,businessClass,firstClass,punctualityRate,dcity,acity,date) values('%s','%s','%s','%s',%s,%s,%s,'%s','%s','%s','%s')"%(info['Airline'],info['FlightNumber'],info['DepartureDate'],info['ArrivalDate'],info['economyClass'],info['businessClass'],info['firstClass'],info['PunctualityRate'],dcity,acity,date)
            print(sql)
            cursor.execute(sql)
            db.commit()
...
if __name__ == "__main__":
    ...
    mm=int(sys.argv[4])
    dd=int(sys.argv[5])
    print(dcity,acity,yyyy,mm,dd)
    ncity=xiecheng(dcity,acity,yyyy,mm,dd)
    print(dcity,acity,ncity)
